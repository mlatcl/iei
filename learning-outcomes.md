...
outcomes:
  ...
  - id: LO4
    ...
    bloom_level: analyse
    week: 4
    notes: "Maxwell (1867); Landauer (1961): erasing one bit costs k_B T ln 2. Feedback work bounded by k_B T I(X;M) (Parrondo/Sagawa–Ueda). Macroscopic engines (car) cannot exploit demon-style feedback — thermal DOF/s dwarfs any bit memory; molecular machines (ATP synthase) operate at the scale where information thermodynamics applies. First rigorous link between decision-making and entropy; multi-information and I+H=C come later (week 8)."
  ...
  - id: LO8
    ...
    bloom_level: analyse
    week: 6
    notes: "Amari & Nagaoka (2000). g_ij = E[∂_i log p · ∂_j log p]. e-flat (natural parameters) and m-flat (mean parameters) coordinate systems — the Legendre pair (θ, η) from week 5. Pythagorean theorem: KL(p||r) = KL(p||q) + KL(q||r) when q is the e/m projection. This metric is Crooks' thermodynamic length (2007): the Fisher–Rao length of a path of equilibrium states. In the slow-driving regime dissipation is bounded by L²/τ. Define length in week 6; interpret as a constraint on intelligence in week 8."
  ...
  - id: LO10
    ...
    bloom_level: analyse
    week: 8
    notes: "Watanabe (1960). I = Σ h_i - H ≥ 0. Mutual information is the pairwise case. With fixed marginal entropies C = Σ h_i, conservation I + H = C. I ~ potential energy (stored correlation); H ~ kinetic energy (free uncertainty). Data-processing inequality (Cover & Thomas Thm 2.8.1): if X→Y→Z is Markov, I(X;Z)≤I(X;Y) — processing cannot create information. Information bottleneck (Tishby, Pereira, Bialek 1999): the prescription inside that fence — minimise I(X;T) subject to I(T;Y)≥R, or min I(X;T)−β I(T;Y). Named in week 3; taught this week; used later in the same session for LO13. Not separate outcomes. The inaccessible-game introduction may appear as light colour, not as a full axiomatic framework."
  ...
  - id: LO11
    ...
    bloom_level: analyse
    week: 8
    notes: "Classically H = 0 ⟹ deterministic (delta distribution). Quantum: pure entangled state |ψ⟩ has S(ρ) = 0 but reduced states ρ_A, ρ_B have S > 0. Forced by asking for I = C with nontrivial marginals under classical probability. Algebraic quantum probability: outcomes no longer primitive."
...
thermodynamics, information theory, maximum entropy, information geometry, and
information-theoretic limits on intelligent agency.
...
(Amari) provides the unifying geometric language. Multi-information, the data-processing
inequality, and the information bottleneck make precise what processing can and cannot do;
those tools, with Landauer and \(\mathcal{L}^2/\tau\), make the superintelligence promise
analogous to perpetual motion and force the classical limit $I=C$ toward von Neumann entropy.
...
the information bottleneck, the Schrödinger bridge.
...
central concept with an LLM using the Socratic habit (curiosity then skepticism) from lecture 1 (thermodynamic,
...
Eight Tuesdays, 13 October – 1 December 2026, 10:00–12:00, FW26, William Gates Building. Each slot is two hours. Class tests take the first ten minutes of lectures 2, 5, 7 and 8 (weeks 2, 5, 7, 8). Week 8 splits the two-hour slot: multi-information / DPI / IB / von Neumann, then probability transport and limits.
...
| 6 | 17 Nov | Fisher metric; thermodynamic length defined (Crooks) | LO8 | W3 released |
| 7 | 24 Nov | MaxEnt as projection; natural gradient; geodesics | LO9 | Q3 (10 min); W3 due; W4 released |
| 8 | 1 Dec | Multi-information; DPI; IB; von Neumann; three geometries; limits on intelligence | LO10–LO13 | Q4 (10 min); W4 due |

## Outcome summary

| ID   | Short label                                           | Bloom level | Week |
|------|-------------------------------------------------------|-------------|------|
| LO1  | Boltzmann decomposition: U, TS, free energy           | Analyse     | 2    |
| LO2  | Shannon entropy ≡ thermodynamic entropy               | Apply       | 3    |
| LO3  | Partition function and thermodynamic quantities       | Apply       | 3    |
| LO4  | Maxwell's demon and Landauer's principle              | Analyse     | 4    |
| LO5  | Maximum entropy principle (Jaynes)                    | Apply       | 5    |
| LO6  | Exponential family as MaxEnt family                   | Analyse     | 5    |
| LO7  | Compare three perspectives on entropy                 | Evaluate    | 5    |
| LO8  | Fisher metric and dually flat geometry                | Analyse     | 6    |
| LO9  | MaxEnt as projection; natural gradient                | Apply       | 7    |
